...
RET_CODE open_ca_entitle_expire_dlg(u32 para1, u32 para2)
{
  if(g_cas_id_flag  == CAS_ID_ADT_MG)
  {
	...
	p_cont = fw_create_mainwin(ROOT_ID_CA_ENTITLE_EXP_DLG,
					   ...
					   ENTITLE_CONT_FULL_W_MG, ENTITLE_CONT_FULL_H_MG,
					   ROOT_ID_INVALID, 0,
					   OBJ_ATTR_ACTIVE, 0);
	if(p_cont == NULL)
		return ERR_FAILURE;
	...
	p_content = ctrl_create_ctrl(CTRL_TEXT, IDC_CONTENT,
					   ...
					   ENTITLE_CONTENT_FULL_W_MG, ENTITLE_CONTENT_FULL_H_MG,
					   p_cont, 0);
  ...
  else
  {
	  ...
	  p_cont = fw_create_mainwin(ROOT_ID_CA_ENTITLE_EXP_DLG,
	                           ENTITLE_CONT_FULL_X, ENTITLE_CONT_FULL_Y,
	                           ENTITLE_CONT_FULL_W_DIVI, ENTITLE_CONT_FULL_H_DIVI,
	                           ROOT_ID_INVALID, 0,
	                           OBJ_ATTR_ACTIVE, 0);
	  if(p_cont == NULL)
	  {
	  ...
	  p_content = ctrl_create_ctrl(CTRL_TEXT, IDC_CONTENT,
	                       ENTITLE_CONTENT_FULL_X, ENTITLE_CONTENT_FULL_Y,
	                       ENTITLE_CONTENT_FULL_W_DIVI, ENTITLE_CONTENT_FULL_H_DIVI,
	                       p_cont, 0);
...
BEGIN_KEYMAP(comm_ca_entitle_expire_keymap, NULL)
  ...
  ON_COMMAND(V_KEY_UP, MSG_FOCUS_UP)
  ON_COMMAND(V_KEY_DOWN, MSG_FOCUS_DOWN)
