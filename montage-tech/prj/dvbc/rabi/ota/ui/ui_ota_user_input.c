...
s32 open_ota_user_input(u32 para1, u32 para2)
{
  ...
  OS_PRINTF("[%s] pid[%x] fre[%d] \n",__FUNCTION__,ota_param.ota_tp.data_pid,ota_param.ota_tp.lockt.tp_freq);
        ...
        switch(i + 1)
        {
          case IDC_OTA_USER_INPUT_TPFREQ:
            ...
            if(ota_param.ota_tp.lock_mode == SYS_DVBC)
			  tp_freq = ota_param.ota_tp.lockc.tp_freq;
            else if(ota_param.ota_tp.lock_mode == SYS_DTMB || ota_param.ota_tp.lock_mode == SYS_DVBT2)
			  tp_freq = ota_param.ota_tp.lockt.tp_freq;

			if(tp_freq< OTA_TPFREQ_MIN)
				tp_freq = OTA_TPFREQ_MIN;
			else if(tp_freq>OTA_TPFREQ_MAX)
				tp_freq = OTA_TPFREQ_MAX;

            nbox_set_num_by_dec(p_item[i], tp_freq);
            break;
