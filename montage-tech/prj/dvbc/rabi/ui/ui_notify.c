...
static u32 g_notify_close_time = NOTIFY_AUTOCLOSE_MS;
...
void ui_set_notify_during(u32 during)
{
	DEBUG(MAIN,INFO,"during = %d g_notify_close_time = %d\n",during,g_notify_close_time);
	g_notify_close_time = during;
}
  ...
  else
  {
    ...
    if ((p_cont = fw_find_root_by_id(ROOT_ID_NOTIFY)) != NULL)//already open
    {
      ...
      else
      {
        ...
        fw_tmr_reset(ROOT_ID_NOTIFY, MSG_EXIT, g_notify_close_time);
      }
    }
    else
    {
...
RET_CODE open_notify(u32 para1, u32 para2)
{
  ...
  fw_tmr_create(ROOT_ID_NOTIFY, MSG_EXIT, g_notify_close_time, FALSE);
#ifdef ENABLE_CA
