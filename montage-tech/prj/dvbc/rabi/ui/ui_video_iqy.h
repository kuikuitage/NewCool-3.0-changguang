...
{
    char  tvQid[IQY_LEN_QPID];                //¾ç¼¯ÆæÆ×ID
    char  vid[IQY_LEN_VID];                   //²¥·ÅÂëÁ÷id
}video_url_info_t;


RET_CODE ui_iqy_video_v_open(u32 para1, u32 para2);
void _ui_iqy_video_v_set_full_stop_states(BOOL states);
/*!
 * Ui video control function
 */
RET_CODE ui_iqy_video_c_create(u8 play_func);
RET_CODE ui_iqy_video_c_play_by_file(u16 *p_file_name, u32 seek_play_time);
RET_CODE ui_iqy_video_c_play_by_url_ex(video_url_info_t *pp_url_list, u32 url_cnt, u32 seek_play_time, int vd,ui_iqy_play_mode_t play_mode);
...
RET_CODE ui_iqy_video_c_set_speed(void);
 ...
 */
RET_CODE ui_iqy_video_m_load(u32 partition);
RET_CODE ui_iqy_video_m_net_load(u16*ip_address_path);

u32 ui_iqy_video_m_get_total(void);
u16 ui_iqy_video_m_get_partition_cnt(void);
void ui_iqy_video_m_get_name_by_idx(u32 idx, u16 *p_file_name, u16 str_len);
void ui_iqy_video_m_get_size_by_idx(u32 idx, u32 *p_size);
u16 * ui_iqy_video_m_get_path_by_idx(u32 idx);
file_type_t ui_iqy_video_m_get_type_by_idx(u32 idx);
RET_CODE ui_iqy_video_m_go_to_dir(u16 idx);
RET_CODE ui_iqy_video_m_back_to_parent_dir(void);
u16 * ui_iqy_video_m_get_cur_path(void);
void ui_iqy_video_m_set_cur_playing_file_by_idx(u32 idx);
void ui_iqy_video_m_set_support_flag_by_idx(u32 idx, BOOL support);
u16 *ui_iqy_video_m_get_cur_playing_file(void);
RET_CODE ui_iqy_video_m_get_next_file_idx(u16 *p_idx);
RET_CODE ui_iqy_video_m_get_prev_file_idx(u16 *p_idx);
u16 ui_iqy_video_m_get_cur_playing_idx(void);
u16 *ui_iqy_video_m_get_cur_playing_file_path(void);
BOOL ui_iqy_video_m_check_cur_playing_file_support(void);

u16 ui_iqy_video_m_get_cur_partition(void);
void ui_iqy_video_m_reset(void);
u16 * ui_iqy_video_m_get_name_ptr_by_idx(u32 idx);
u16 ui_iqy_video_m_get_video_icon(u16 *p_filename);
RET_CODE ui_iqy_video_m_find_idx_by_cur_play_file(u32 *p_idx);

void ui_iqy_video_m_save_total_time(video_play_time_t *time);
void ui_iqy_video_m_get_saved_total_time(video_play_time_t *time);
void ui_iqy_video_m_save_resolution(rect_size_t *rect);
void ui_iqy_video_m_get_saved_resolution(rect_size_t *rect);
BOOL ui_iqy_video_file_is_enter_dir(void);
handle_t get_rsc_handle();
void ui_iqy_video_m_rsc_init();
void ui_iqy_video_m_rsc_release();
